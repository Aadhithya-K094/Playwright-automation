...
test("login page", async ({ page }) => {
  ...
  await expect(page).toHaveURL("https://emis-staging.tnsed.com/auth/login?returnUrl=%2Fdashboard");

  //verify title
  const pagetitle = await page.title();
  console.log("The page title is:", pagetitle);

  await expect(page).toHaveTitle("EMIS");

  //check logo visible
  const logo =page.getByRole('img').nth(1)
  ...
  const string = await page.getByText('Educational Management');
  await expect(string).toBeVisible();
  console.log("This is string:",string);


  // // invalid User id with space
  // const userid1 =page.getByRole('textbox', { name: 'User Name' });
  // await userid1.hover();
  // await page.waitForTimeout(500);
